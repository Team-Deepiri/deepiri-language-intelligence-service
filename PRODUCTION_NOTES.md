...
LIS does not persist embeddings and does not enable `pgvector`. Chunk vectors are produced by Cyrex and stored in Milvus after LIS publishes `document.vectorize` on the bus.
...
3. Counts all records
4. Deletes the Document
5. Verifies cascade deleted related records (chunks, jobs, results)
6. Exits 0 if successful, non-zero if failed
